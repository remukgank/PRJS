...
## 1. 7 Fungsi + Test Coverage (wajib 6 fungsi + 1 router)

| Fungsi | Baris | Test scenario |
|--------|-------|---------------|
| `handleGofileUrl` direct (1344-1580) | GoFile direct: `kjny03unc` → judul Kaifuku auto-detect, part 3, caption S-episode benar |
| `handleGofileBatch` (1584-1657) | Batch multi-file: loop per-file tanpa judul prompt, `batchPart` per file, tidak throw kosong list |
| `handlePixeldrainUrl` (1658-1806) | Pixeldrain: `GKsTIeO-S2-5` → `pixPart` = 5 via `sami?.episode ?? parseSamehadakuFilename ?? extract` |
| `handleFiledonUrl` (1807-1892) | Filedon: `TSSDK-S2-P2-1` → `partN = fdSame?.episode` + alias `tss→tssdk`, suffix S2P2 |
| `handleGdriveUrl` (1957-~2120) | GDrive: `TsSDKMGnoKh-FULLHD` → `titleForMedia` S-suffix anti-dobel, `epNum` benar |
| `downloadSamehadakuFile` (1893-1911) | Router: server gofile→handleGofileUrl, pixeldrain→handlePixeldrainUrl, filedon→handleFiledonUrl, server tak dikenal→ pesan fallback |
| `handleUcDriveUrl` (1175-1245) | UC Drive: share invalid → pesan error, share valid → download + kirim (mock `getShareInfo`) |
...
- 5 fungsi lain (Batch, Filedon, `downloadSamehadakuFile`, GDrive, UcDrive): **TIDAK** akses Map apa pun secara langsung
...
| **E4b** | `handlePixeldrainUrl`, `handleFiledonUrl`, `handleGdriveUrl`, `handleUcDriveUrl`, `downloadSamehadakuFile` (router terakhir, setelah yang dipanggil stabil) | Medium-High |

Tidak dipecah per-fungsi (overhead 7 branch/commit terlalu besar untuk benefit), tapi tidak juga 1 commit raksasa.
