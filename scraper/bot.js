...
_downloadHandlers.initDownload({
  bot,
...
async function handleUcDriveUrl(chatId, text) {
  return _downloadHandlers.handleUcDriveUrl(chatId, text);
}
...
async function handleGofileUrl(chatId, url, customTitle = null) {
  return _downloadHandlers.handleGofileUrl(chatId, url, customTitle);
}

async function handleGofileBatch(chatId, urls) {
  return _downloadHandlers.handleGofileBatch(chatId, urls);
}
...
async function handlePixeldrainUrl(chatId, url, customTitle = null) {
  return _downloadHandlers.handlePixeldrainUrl(chatId, url, customTitle);
}
...
function pad(n) { return _vidaraHandlers.pad(n); }
function buildResolveVideoUrl(session) { return _vidaraHandlers.buildResolveVideoUrl(session); }
async function actionVidaraPerEp(chatId, session) { return _vidaraHandlers.actionVidaraPerEp(chatId, session); }
async function actionVidaraMerge10(chatId, session) { return _vidaraHandlers.actionVidaraMerge10(chatId, session); }
async function actionVidaraAndTelegramMerge10(chatId, session) { return _vidaraHandlers.actionVidaraAndTelegramMerge10(chatId, session); }
async function actionVidaraAndTelegramPerEp(chatId, session) { return _vidaraHandlers.actionVidaraAndTelegramPerEp(chatId, session); }
...
bot.on('callback_query', safeHandler('callback')(async (query) => {
  ...
  // (sam_next dihapus — lanjut via daftar episode `sam_ep` di pesan yang sama)
